...
use crate::{
    ...
    database::models::user::{NewUser, User},
    database::setup::DynamoDbClient,
};
use axum::{
    extract::State,
...
pub async fn register_handler(
    ...
    match User::register(new_user, &dynamo_client).await {
        Ok(_) => match User::fetch_by_username(&username, &dynamo_client).await {
            Ok(Some(user)) => auth
